...
describe("compact public search index", () => {
  it("indexes metadata without requiring article HTML at query time", async () => {
  ...
  it("finds articles by title or category text", async () => {
    const results = await searchPublicContent("robot vacuum");
    expect(results.length).toBeGreaterThan(0);
    expect(results[0].title.toLowerCase()).toContain("robot");
    expect(searchIndex("robot vacuum", await getSearchIndex()).length).toBe(results.length);
  });
});
